...
迁移包只包含共享账号库密文（或已删除账号库的持久墓碑）、项目目录和历史报告。它明确不包含成员、`local_accounts`、登录密码、会话、环境密钥、Cookie 或初始化令牌，所以目标站仍需在 `/setup` 创建自己的所有者和管理员。

当前导出格式为 v4：它显式保留账号库的删除状态、revision 和时间，避免迁移后被未同步删除的旧客户端复活。新导入器仍兼容历史 v2/v3 包；旧导入器会拒绝 v4，不会将新墓碑静默当作空库。
